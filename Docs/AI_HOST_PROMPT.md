...
下面这段内容可以直接提供给 AI 编码助手，用于在我的上位机项目中生成 STM32 数字电源通信模块。  
目标是让 AI 严格按照当前固件实现的串口协议完成“读取实时状态、写入参数、控制开关、处理 ACK/NACK、解析 TLV”的功能。

---

## 可直接使用的提示词

你现在要为一个上位机项目实现“STM32 数字电源通信接口层”。  
请严格按照下面的协议和约束生成代码，不要自行改协议，不要擅自修改字段定义，不要引入与协议无关的抽象。
...
如果我要求单文件版本，则把这些内容合并为一个类文件，但仍然保留清晰分层。
...
2. 如果响应 `seq` 不一致，应报错
3. 等待请求响应时允许先收到 `REPORT`，处理后继续等待
4. 如果响应命令不是 `ACK` 或 `NACK`，且不是可处理的 `REPORT`，应报错
5. 如果响应为 `NACK`，应抛异常
6. 允许一次 `READ` 返回多个 TLV
7. 解析 TLV 时，如果结构不完整，应抛异常
8. 如果某个请求的数据项未在响应中出现，应抛异常或给出明确提示
...
3. `decode_fault_flags(mask)`  
   输出故障名称列表

4. `poll_status(interval=0.5)`  
   周期读取状态，适合 GUI 刷新

5. `pretty_print_status(status)`  
   用于调试打印
...
请输出完整、可运行的 Python 代码。  
如果是多文件，请分别给出每个文件内容。  
如果是单文件，请给出一个可直接运行的完整版本。  
同时附带一个最小调用示例。
...
如果你要把这段提示词嵌入到你自己的 AI 上位机工程里，建议拆成三部分：

1. `系统提示`
   - 说明 AI 的身份是“串口协议模块开发助手”
   - 强调“严格遵守协议，不自行改协议”

2. `协议上下文`
   - 放本文件中的“帧协议、DataType、单位、读写规则”

3. `任务提示`
   - 例如：
     - “请为 PySide6 项目生成一个串口通信服务类”
     - “请生成一个后台轮询线程”
     - “请生成一个 QML 可绑定的数据模型”
...
### 示例 2：生成 PySide6 后台通信线程

请基于上述协议，为 PySide6 项目生成：
...
### 示例 3：生成 tkinter 调试工具

请基于上述协议生成一个 tkinter 调试工具，要求：

- 支持选择串口
- 打开/关闭连接
- 设置输出电压和电流
- 打开/关闭输出
- 实时显示 Vin/Vout/Iout/温度/故障状态
...
- 协议示例文件：`Docs/tvl_host_example.py`
- 固件通信实现：`APP/Src/app_tvlcom.c`
- 固件业务实现：`APP/Src/app_power.c`

但要求 AI 以“上位机实现”为主，不要照搬 STM32 HAL 代码。
