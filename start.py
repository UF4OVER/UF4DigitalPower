...
class DynamicIsland(QFrame):
    ...
    def _applyStyle(self, level: str = None):
        ...
        self.setStyleSheet(f"""
            QFrame#DynamicIsland {{ background: {bg}; border: 1px solid {border}; border-radius: 17px; }}
            QLabel#DynamicIslandIndicator {{ background: {color}; border-radius: 4px; }}
            QLabel#DynamicIslandTitle {{ color: {titleColor}; font-size: 12px; font-weight: 600; }}
            QLabel#DynamicIslandContent {{ color: {contentColor}; font-size: 12px; }}
        """)
