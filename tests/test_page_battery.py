...
from App.Pages.page_battery import BatteryPage, BatterySnapshot
...
class BatteryPageTests(unittest.TestCase):
    ...
    def test_snapshot_updates_visual_sections(self):
        ...
        try:
            ...
            self.assertEqual(page.cellGrid.rowCount(), 2)
            self.assertEqual(page.cellGrid.columnCount(), 2)
            self.assertIn("3.812", page.cellCards[0].voltageLabel.text())
            self.assertIn("电量 77.1", page.cellCards[0].socLabel.text())
