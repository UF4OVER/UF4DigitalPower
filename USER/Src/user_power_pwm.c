...
void UserPowerPwm_Stop(user_power_status_t *status)
{
    ...
    __HAL_HRTIM_SETCOMPARE(&hhrtim1, HRTIM_TIMERINDEX_TIMER_D, HRTIM_COMPAREUNIT_1, USER_PWR_BOOST_MIN_COMPARE);

    if (status != NULL)
    {
        status->pwm_a_compare = USER_PWR_PWM_PERIOD;
        status->pwm_d_compare = USER_PWR_BOOST_MIN_COMPARE;
    }
}
...
void UserPowerPwm_ApplyDuty(user_power_topology_t topology, float duty, user_power_status_t *status)
{
    ...
    duty = user_pwr_clamp(duty, 0.02f, 0.95f);
    ...
    case USER_POWER_TOPOLOGY_BUCK:
        ...
        compare_d = USER_PWR_BOOST_MIN_COMPARE;
        break;
